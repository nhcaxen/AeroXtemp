...
export default function TempMailTab() {
  ...
  // Generate a random email address and mailbox on Mail.tm
  const generateNewEmail = async () => {
    ...
    try {
      ...
      if (!domainRes.ok) {
        throw new Error(`Failed to fetch email domains (${domainRes.status})`);
      }
      const domainsData = await domainRes.json();
      const activeDomains = (domainsData["hydra:member"] || []) as MailTmDomain[];
      const usableDomain = activeDomains.find(d => d.isActive) || activeDomains[0];
      
      if (!usableDomain) {
        throw new Error("No active email domains available at this time.");
      }

      const userPart = "bb_" + Math.random().toString(36).substring(2, 11);
      const generatedEmail = `${userPart}@${usableDomain.domain}`;
      const generatedPassword = Math.random().toString(36).substring(2, 12) + "A1!";

      const createRes = await fetchWithRetry("https://api.mail.tm/accounts", {
        method: "POST",
        headers: {
      ...
      if (!createRes.ok) {
        throw new Error(`Failed to register account on mail server (${createRes.status})`);
      }
      ...
      if (!tokenRes.ok) {
        throw new Error(`Failed to retrieve security token (${tokenRes.status})`);
      }
  ...
  const handleViewMessage = async (msgId: string) => {
    ...
    try {
      const res = await fetchWithRetry(`https://api.mail.tm/messages/${msgId}`, {
        headers: {
          "Authorization": `Bearer ${token}`
        }
      });
      ...
      if (res.ok) {
        const data = (await res.json()) as MailTmFullMessage;
  ...
  return (
    <div className="flex flex-col gap-4 px-4 pt-4 pb-8 select-none">
      {/* Tab Header Banner */}
      <div className="bg-gradient-to-r from-pink-600/10 via-cyber-purple/10 to-transparent p-4 rounded-2xl border border-white/[0.04] relative overflow-hidden">
      ...
      <div className="flex bg-void-black/80 p-1 rounded-xl border border-white/[0.04] shrink-0">
        <button
          ...
          className={`flex-1 py-2 rounded-lg text-xs font-bold uppercase transition-all cursor-pointer ${
            mode === "active"
              ? "bg-gradient-to-tr from-cyber-purple to-pink-600 text-white shadow-md font-extrabold"
          ...
          <div className="p-4 rounded-2xl bg-dark-surface border border-white/[0.04] shadow-md flex flex-col gap-3">
            ...
            <div className="grid grid-cols-2 gap-2">
              ...
              <button
                ...
                className="py-2.5 rounded-xl bg-gradient-to-r from-pink-600 to-cyber-purple hover:opacity-90 active:scale-95 text-xs font-black uppercase text-white shadow-lg transition-all flex items-center justify-center gap-1.5 cursor-pointer disabled:opacity-40"
              ...
              <div className="flex flex-col gap-4 animate-fade-in text-left">
                ...
                <div className="bg-void-black p-3.5 rounded-xl border border-white/[0.03] flex flex-col gap-1.5 select-all">
                  ...
                  <div>
                    ...
                    <span className="text-xs font-semibold text-pink-300 block">{selectedMessage.subject || "(No Subject)"}</span>
                  </div>
              ...
              <div className="flex flex-col gap-3">
                <div className="flex items-center justify-between mb-1 px-1">
                  ...
                  {messages.some(m => !m.seen) && (
                    <span className="text-[9px] text-pink-400 font-bold bg-pink-500/10 px-2 py-0.5 rounded-full animate-pulse border border-pink-500/20">
                ...
                {loadingContent ? (
                  ...
                  <div className="bg-void-black/80 py-12 px-6 rounded-xl border border-white/[0.02] text-center select-none">
                    <div className="w-10 h-10 rounded-full bg-pink-500/5 flex items-center justify-center text-pink-400/40 mx-auto mb-3 border border-pink-500/10">
                  ...
                  <div className="flex flex-col gap-2 max-h-96 overflow-y-auto scrollbar-none pr-0.5">
                    {messages.map((msg) => (
                      ...
                      >
                        <div className="absolute top-0 left-0 w-1 h-full bg-pink-500/50 rounded-r opacity-0 group-hover:opacity-100 transition-all" />
                        ...
                        <div className="flex flex-col gap-1 max-w-[80%]">
                          ...
                          <span className={`text-xs font-bold transition-all truncate ${msg.seen ? "text-white" : "text-pink-300"}`}>
                        ...
                        <div className="shrink-0 p-1.5 rounded-lg bg-pink-500/10 text-pink-400 hover:text-white transition-colors">
              ...
              {selectedRecoveredMessage ? (
                ...
                <div className="flex flex-col gap-3">
                  <div className="flex items-center justify-between border-b border-white/[0.04] pb-3">
                    ...
                    >
                      <RefreshCw className={`w-4 h-4 ${loadingRecoveredMessages ? "animate-spin text-pink-500" : ""}`} />
                    </button>
                  </div>
              ...
              {loadingMailboxes ? (
                ...
                <div className="bg-dark-surface py-12 px-6 rounded-2xl border border-white/[0.04] text-center select-none flex flex-col items-center gap-4">
                  ...
                  <button
                    ...
                    className="px-4 py-2 rounded-xl bg-gradient-to-tr from-cyber-purple to-pink-600 text-white text-xs font-bold uppercase cursor-pointer hover:scale-105 active:scale-95 transition-all"
                  >
                    Create Temp Mail
                  </button>
                </div>
